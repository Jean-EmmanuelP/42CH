...
export default function Social() {
  ...
  async function getFriends() {
    const request = await axios.post("http://localhost:3333/user/get_friends/", JSON.stringify({ username: sessionStorage.getItem('username') }), { headers: { 'Content-Type': 'application/json' } })
  ...
  async function getOnlineUsers() {
    const request = await axios.get("http://localhost:3333/user/get_online_users/")
